--[[
    ========================================================================
    ⚡ AJIZ HUB - RUNAWAYS (INTEGRATED COMPATIBILITY WRAPPER) ⚡
    ========================================================================
--]]
...
-- ========================================================================
-- 🎨 DYNAMIC ORION/RAYFIELD TO AJIZLIB WRAPPER
-- ========================================================================
local wrapperScript = [[
    local AjizLib = (function()
--[[
...
return AjizLib
    ...
    function lib:MakeNotification(cfg)
        AjizLib:Notify(cfg.Name or cfg.Title or "Ajiz Hub", cfg.Content or cfg.Text or "", cfg.Time or cfg.Duration or 3)
    end
    lib.Notify = lib.MakeNotification

    function lib:Init() end
    function lib:Destroy() end

    function lib:CreateWindow(cfg)
        local title = cfg.Name or cfg.Title or cfg.Text or "Ajiz Hub"
        ...
        local function wrapTab(tabName)
            local tabWrapper = {}
            
            function tabWrapper:AddToggle(name, default, callback)
                local tName = name
                local tDefault = default
                local tCallback = callback
                if type(name) == "table" then
                    tName = name.Name or name.Title
                    tDefault = name.Default
                    tCallback = name.Callback
                end
                panel:AddToggle(tName, tDefault or false, tCallback)
            end
            
            function tabWrapper:AddButton(name, callback)
                local bName = name
                local bCallback = callback
                if type(name) == "table" then
                    bName = name.Name or name.Title
                    bCallback = name.Callback
                end
                panel:AddButton(bName, bCallback)
            end
            
            function tabWrapper:AddSlider(name, min, max, default, callback)
                local sName = name
                local sMin = min
                local sMax = max
                local sDefault = default
                local sCallback = callback
                if type(name) == "table" then
                    sName = name.Name or name.Title
                    sMin = name.Min
                    sMax = name.Max
                    sDefault = name.Default or name.Def
                    sCallback = name.Callback
                end
                -- Map Slider to a Toggle for simple activation
                panel:AddToggle(sName, false, function(state)
                    if state then
                        sCallback(sMax or 150)
                    else
                        sCallback(sDefault or 16)
                    end
                end)
            end

            function tabWrapper:AddLabel(name)
                print("[AJIZ WRAPPER] Label added: " .. tostring(type(name) == "table" and (name.Name or name.Text) or name))
            end

            function tabWrapper:AddDropdown(name, list, default, callback)
                print("[AJIZ WRAPPER] Dropdown added: " .. tostring(type(name) == "table" and name.Name or name))
            end
            
            tabWrapper.CreateToggle = tabWrapper.AddToggle
            tabWrapper.CreateButton = tabWrapper.AddButton
            tabWrapper.CreateSlider = tabWrapper.AddSlider
            tabWrapper.CreateLabel = tabWrapper.AddLabel
            tabWrapper.CreateDropdown = tabWrapper.AddDropdown
            
            return tabWrapper
        end
        
        function wrapper:AddTab(tabCfg)
            local tabName = type(tabCfg) == "table" and (tabCfg.Name or tabCfg.Title) or tostring(tabCfg)
            return wrapTab(tabName)
        end
        
        wrapper.MakeTab = wrapper.AddTab
        wrapper.CreateTab = wrapper.AddTab
...
-- ========================================================================
-- 🏝️ HTTPGET HOOK FOR COMPATIBILITY INJECTION
-- ========================================================================
local oldHttpGet
if hookfunction then
    pcall(function()
        oldHttpGet = hookfunction(game.HttpGet, newcclosure(function(self, url, ...)
            ...
            if lurl:find("orion") or lurl:find("rayfield") then
        ...
        mt.__index = newcclosure(function(self, key)
            if key == "HttpGet" then
                return function(self, url, ...)
                    ...
                    return oldIndex(self, "HttpGet")(self, url, ...)
                end
            end
...
-- ========================================================================
-- ⚡ EXECUTE ORIGINAL OBFUSCATED SCRIPT
-- ========================================================================
loadstring(game:HttpGet("https://raw.githubusercontent.com/adeel-abid-09/Ajiz-Hub/main/dist/Runaways_Obfuscated.lua"))()
