--[[
    ========================================================================
    ⚡ AJIZ HUB - BARRY'S PRISON ESCAPE (MODULAR GAME LOGIC) ⚡
    ========================================================================
--]]
...
-- Teleport player safely to a CFrame coordinate
...
-- Find any item named Keycard/Key in Workspace and teleport player to it
...
-- Auto Win: Teleports player directly to the final stage/badge
local function autoWin()
    local checkpoints = getCheckpoints()
    if #checkpoints > 0 then
        -- Teleport to the highest checkpoint stage found
        local finalStage = checkpoints[#checkpoints]
        safeTeleport(finalStage.part.CFrame)
        return true
    end
    
    -- Fallback: Look for final pads/helicopter/exit signs
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("BasePart") then
            local name = obj.Name:lower()
            if name:find("helicopter") or name:find("escape") or name:find("win") or name:find("end") or name:find("exit") then
...
-- 1. Barry Boss/NPC ESP Loop
task.spawn(function()
    while true do
        task.wait(1.5)
        pcall(function()
            -- Clear old highlights if toggle is off
            if not _G.BarryESP then
                for npc, hl in pairs(BarryHighlights) do
                ...
                for _, obj in ipairs(Workspace:GetDescendants()) do
                    if obj:IsA("Model") and not Players:GetPlayerFromCharacter(obj) then
                        ...
                        if name:find("barry") or name:find("boss") or name:find("guard") or name:find("npc") then
                            if not BarryHighlights[obj] or not BarryHighlights[obj].Parent then
...
Panel:AddToggle("Speed Boost (Fast Walk)", false, function(state)
    _G.WalkSpeed = state and 60 or 16
    pcall(function()
        local char = LocalPlayer.Character
        local hum = char and char:FindFirstChildWhichIsA("Humanoid")
...
Panel:AddToggle("Super Jump", false, function(state)
    _G.JumpPower = state and 110 or 50
    pcall(function()
        local char = LocalPlayer.Character
        local hum = char and char:FindFirstChildWhichIsA("Humanoid")
        if hum then
...
Panel:AddButton("Teleport to Keycard / Key", function()
    local success = grabKeycard()
    if success then
        AjizLib:Notify("Teleported", "Teleported directly to Keycard / Key!", 3)
    else
        AjizLib:Notify("Not Found", "No Keycard or Keys found in this area.", 3)
    end
end)

Panel:AddButton("Instant Win / Auto Escape", function()
    local success = autoWin()
    if success then
...
-- Stage Checkpoints teleport generator (adds quick skip buttons)
task.spawn(function()
    task.wait(2)
    pcall(function()
        local checkpoints = getCheckpoints()
        if #checkpoints > 0 then
            -- Add buttons for intermediate stages (first, middle, last)
            local half = math.floor(#checkpoints / 2)
            
            Panel:AddButton("Teleport to Stage 1", function()
                safeTeleport(checkpoints[1].part.CFrame)
                AjizLib:Notify("Teleported", "Teleported to Stage 1 Checkpoint", 3)
            end)
            
            if half > 1 and half < #checkpoints then
                Panel:AddButton("Teleport to Stage " .. tostring(checkpoints[half].num), function()
                    safeTeleport(checkpoints[half].part.CFrame)
                    AjizLib:Notify("Teleported", "Teleported to Stage " .. tostring(checkpoints[half].num), 3)
                end)
            end
            
            Panel:AddButton("Teleport to Stage " .. tostring(checkpoints[#checkpoints].num), function()
                safeTeleport(checkpoints[#checkpoints].part.CFrame)
                AjizLib:Notify("Teleported", "Teleported to Final Checkpoint", 3)
            end)
        end
    end)
end)

AjizLib:Notify("Ajiz Hub", "Barry's Prison Escape Script Loaded!", 3)
