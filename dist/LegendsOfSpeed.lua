--[[
    ========================================================================
    ⚡ AJIZ HUB - LEGENDS OF SPEED (MODULAR GAME LOGIC) ⚡
    ========================================================================
--]]
...
-- 1. Auto Farm Orbs & Gems Loop (Optimized 0.3s Scan Rate)
task.spawn(function()
    while true do
        task.wait(0.3)
        if _G.AutoFarm then
            pcall(function()
                ...
                if hrp and orbFolder then
                    for _, mapFolder in ipairs(orbFolder:GetChildren()) do
                        for _, obj in ipairs(mapFolder:GetChildren()) do
                            if obj:IsA("TouchTransmitter") or obj:FindFirstChildOfClass("TouchTransmitter") then
...
-- 2. Auto Collect Sky Hoops Loop (Optimized 0.4s Scan Rate)
task.spawn(function()
    while true do
        task.wait(0.4)
        if _G.AutoHoops then
            pcall(function()
                local char = LocalPlayer.Character
                local hrp = char and char:FindFirstChild("HumanoidRootPart")
                local hoops = Workspace:FindFirstChild("Hoops") or Workspace:FindFirstChild("hoops")
                
                if hrp and hoops then
                    for _, mapFolder in ipairs(hoops:GetChildren()) do
                        for _, hoop in ipairs(mapFolder:GetChildren()) do
...
-- 3. Auto Rebirth Loop (1s Check Rate)
task.spawn(function()
    while true do
        task.wait(1.0)
        if _G.AutoRebirth then
            pcall(function()
                ...
                local rebirthEvent = rEvents and rEvents:FindFirstChild("rebirthEvent")
...
Panel:AddToggle("Auto Collect Orbs (Speed/Gems)", false, function(state)
    _G.AutoFarm = state
end)

Panel:AddToggle("Auto Collect Hoops (Gems)", false, function(state)
    _G.AutoHoops = state
end)
...
Panel:AddToggle("Speed Boost (Fast Walk)", false, function(state)
    _G.WalkSpeed = state and 200 or 16
    pcall(function()
        local char = LocalPlayer.Character
        local hum = char and char:FindFirstChildWhichIsA("Humanoid")
...
Panel:AddToggle("Super Jump", false, function(state)
    _G.JumpPower = state and 250 or 50
    pcall(function()
        local char = LocalPlayer.Character
        local hum = char and char:FindFirstChildWhichIsA("Humanoid")
        if hum then
...
pcall(function()
    print("[AJIZ SYSTEM] Initializing Legends of Speed Hub...")
    local orbs = Workspace:FindFirstChild("orbFolder")
    local hoops = Workspace:FindFirstChild("Hoops") or Workspace:FindFirstChild("hoops")
    print("[AJIZ SYSTEM] orbFolder status: " .. (orbs and "Found" or "Not Found"))
    print("[AJIZ SYSTEM] hoops status: " .. (hoops and "Found" or "Not Found"))
end)

AjizLib:Notify("Ajiz Hub", "Legends of Speed Script Loaded!", 3)
